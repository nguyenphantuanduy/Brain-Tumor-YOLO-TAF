...
class BrainTumorWrapper:
    ...
    def gt_assign(self, x, y, w, h, img_size = img_size, strides = strides):
        ...
        if size <= s0:
            ...
            if part < 1/3:
                ...
                assign.append((1, self.cell_assign(x, y, w, h, s1, img_size, 0.2)))
                assign.append((2, self.cell_assign(x, y, w, h, s2, img_size, 0.2)))
            else:
                assign.append((2, self.cell_assign(x, y, w, h, s2, img_size, 0.2)))
        else:
            assign.append((2, self.cell_assign(x, y, w, h, s2, img_size, 0.2)))
            ...
            for imgs, targets in tqdm(train_loader, desc=f"Epoch {epoch+1}/{epochs}"):
                ...
                for idx, target in zip(range(len(targets)), targets):
                    ...
                    for bbox in target:
                        ...
                        assign_list = self.gt_assign(x, y, w, h, self.img_size, strides=self.strides)
    ...
    def evaluate(self, val_loader, verbose=True):
        ...
        with torch.no_grad():
            for imgs, targets in tqdm(val_loader, desc="Evaluating", disable=not verbose):
                preds_for_metric = []
                gts_for_metric = []
                imgs = imgs.to(self.device)
                outputs = self.model(imgs)
                for idx, target in zip(range(len(targets)), targets):
                    ...
                    for bbox in target:
                        ...
                        reg_tensor = torch.tensor([x, y, w, h], device=self.device, dtype=torch.float32)
                        assign_list = self.gt_assign(x, y, w, h, img_size=self.img_size, strides=self.strides)
                        for fmap_idx, cells in assign_list:
                            for i, j, iou in cells:
                                assigned_cells.append((fmap_idx, i, j, iou, cls, reg_tensor))

                    gt_boxes_tensor = torch.stack(gt_boxes).to(self.device) if len(gt_boxes) > 0 else torch.zeros((0,4), device=self.device)
                    gt_labels_tensor = torch.tensor(gt_labels, device=self.device, dtype=torch.long) if len(gt_labels) > 0 else torch.zeros((0,), device=self.device, dtype=torch.long)
