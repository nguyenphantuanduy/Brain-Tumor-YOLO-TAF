...
import os
import cv2
from src.model.LossWrapper import NegClsLoss
import torch.nn.functional as F
from src.utils import PrecisionRecall


class BrainTumorWrapperv2:
    def __init__(self, model, optimizer=None, reg_loss = None, cls_loss = None, objness_loss = None, device=None, CKPT_PATH=None, num_classes = 4, strides = strides, img_size = img_size, scheduler = None):
        ...
        self.neg_cls_loss = NegClsLoss(self.num_classes).to(self.device)
        ...
        for epoch in range(epochs):
            ...
            for paths, imgs, targets in tqdm(train_loader, desc=f"Epoch {epoch+1}/{epochs}"):
                ...
                for idx, target in zip(range(len(targets)), targets):
                    ...
                    neg_cls = self.neg_cls_loss(output_no_gt[5:, :])
                    sample_loss += loss_has_gt + 1/3 * (obj_loss_no_gt + neg_cls)
                    # print(output_has_gt)
                    # print("hhhhhhhhhhhh")
    ...
    def evaluate(self, val_loader, verbose=True):
        ...
        if verbose:
            # precision = map_results['precision'].tolist()
            # recall = map_results['recall'].tolist()
