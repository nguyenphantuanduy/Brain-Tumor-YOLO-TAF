...
from src.model.BrainTumorWrapperv4 import BrainTumorWrapperv4
from ultralytics.utils.loss import VarifocalLoss
from src.config import *
from src.model.LossWrapper import *
class MyBrainTumorWrapperv4:
    def __init__(self, model, device=None, CKPT_PATH=None, img_size=img_size, num_classes=4):
        self.wrapper = BrainTumorWrapperv4(model = model, device=device, CKPT_PATH=CKPT_PATH, num_classes = num_classes, img_size = img_size)
        ...
        optimizer_accel = optim.SGD(params=model.parameters(), lr=1e-3, momentum=0.9)
    ...
    def img_predict(self, img, class_names=class_names):
        return self.wrapper.img_predict(img, class_names)
